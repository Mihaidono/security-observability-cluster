...
```bash
./tfstage policies init -reconfigure -backend-config=backend.hcl
./tfstage policies plan
./tfstage policies apply
```

The second var file is generated by the backend control plane and contains the live policy configuration edited through the UI.
