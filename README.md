# Isolens

Isolens is a Terraform-driven EKS lab with an operator control plane.

## Documentation

- [docs/README.md](docs/README.md)
- [docs/repository-overview.md](docs/repository-overview.md)
- [docs/backend-control-plane.md](docs/backend-control-plane.md)
- [docs/frontend-operator-ui.md](docs/frontend-operator-ui.md)
- [docs/testing-and-validation.md](docs/testing-and-validation.md)
- [docs/aws-iam-policies.md](docs/aws-iam-policies.md)
- [docs/contributing.md](docs/contributing.md)

## Repository Layout

- `backend/`
- `frontend/`
- `docker/postgres-init/`
- `infrastructure/stages/bootstrap/`
- `infrastructure/stages/core/`
- `infrastructure/stages/platform/`
- `infrastructure/stages/policies/`
- `infrastructure/stages/applications/`
